...
class ChordGtr(FretBoardGtr):
            ...
            for i in range(len(dot)):
                if nbdot[i] == 1:
                    self.dwg.add(
                        self.dwg.circle(
                            (
                                (len(self.tuning) / 2 + 1 / 2) * self.wf + self._ol,
                                (1.5 + dot[i]) * self.hf + self._ol,
                            ),
                            r=self.dot_radius,
                            fill=self.dot_color,
                ...
                if nbdot[i] == 2:
                    ...
                    self.dwg.add(
                        self.dwg.circle(
                            (
                                (len(self.tuning) / 2 + 1.5) * self.wf + self._ol,
                                (1.5 + dot[i]) * self.hf + self._ol,
                            ),
                            r=self.dot_radius,
                            fill=self.dot_color,
    ...
    def createfretboard(self):
                ...
                self.dwg.add(
                    self.dwg.line(
                        start=(self.wf + self._ol, (self.hf) * (i + 1) + self._ol),
                            ...
                            (self.wf) * (len(self.tuning)) + self._ol,
                            (self.hf) * (1 + i) + self._ol,
                        ),
        ...
        if self.string_same_size == False:
            string_size_list = [
                ((self.string_size) - i / 4) for i in range(len(self.tuning))
            ]
    ...
    def nut(self):
        ...
        if self.string_same_size == False:
            self.dwg.add(
                self.dwg.line(
                    start=(
                        self.wf + self._ol - ((self.string_size)) / 2,
                        (self.hf) * (1) + self._ol,
                    ),
                    end=(
                        (self.wf) * (len(self.tuning))
                        + self._ol
                        + ((self.string_size) - len(self.tuning) / 4) / 2,
                        (self.hf) * (1) + self._ol,
                    ),
        ...
        if max(fretfing) > 4:
            for i in range(len(self.tuning)):
                ...
                t = svgwrite.text.Text(
                    ...
                    insert=(X, Y),
                    dy=["0.3em"],
                    font_size=self.fontsize_bottom_tuning,
                    font_weight="normal",
