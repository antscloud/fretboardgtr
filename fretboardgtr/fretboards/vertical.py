...
general_default = FretBoardGeneralConfig(fret_height=50, fret_width=50)


class VerticalFretBoardContainer(FretBoardLike):
    def __init__(
        self,
        ...
        self.config = (
            config if config is not None else FretBoardConfig(general=general_default)
        )
    ...
    def add_background(self) -> None:
        ...
        y = self.config.general.y_start + open_fret_height
    ...
    def add_frets(self) -> None:
        ...
        for fret_no in range(first_fret, number_of_frets + 2):
            ...
            fret = Fret((y_start, y), (x_end, y), config=self.config.frets)
    ...
    def add_strings(self) -> None:
        ...
        for i, _ in enumerate(self.tuning):
            start = (
                self.config.general.x_start + (self.config.general.fret_width) * (i),
                y_start,
            )
            end = (
                self.config.general.x_start + (self.config.general.fret_width) * (i),
    ...
    def add_tuning(self) -> None:
        ...
        for i, note in enumerate(self.tuning):
            x = self.config.general.x_start + self.config.general.fret_width * (i)
    ...
    def add_note(self, string_no: int, note: str, root: Optional[str] = None) -> None:
        ...
        string_root = self.tuning[string_no]
    ...
    def add_fingering(
        ...
        for string_no, finger_position in enumerate(fingering):
            string_note = self.tuning[len(self.tuning) - 1 - string_no]
            if finger_position is None:
                x = self.config.general.y_start + self.config.general.fret_height * (
                    1 / 2
                )
                y = self.config.general.x_start + (self.config.general.fret_width) * (
                    string_no
                )
                position = (x, y)
